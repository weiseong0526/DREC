...
(function () {
  ...
  const STORAGE_PRODUCT_VIEWS = "pp:productPageViews";
  ...
  const allProducts = () => window.PP_STORE?.getProducts?.() || window.PP_DATA?.products || [];
  const findProduct = (id) => allProducts().find((p) => p.id === id);

  const getProductPageViews = () => {
    try {
      return JSON.parse(localStorage.getItem(STORAGE_PRODUCT_VIEWS) || "{}");
    } catch {
      return {};
    }
  };

  /** Increment when a product detail page is opened (per page load, this browser). */
  const recordProductView = (productId) => {
    if (!productId) return;
    try {
      const map = getProductPageViews();
      map[productId] = (Number(map[productId]) || 0) + 1;
      localStorage.setItem(STORAGE_PRODUCT_VIEWS, JSON.stringify(map));
    } catch {
  ...
  const renderDrawer = () => {
    ...
    if (cart.length === 0) {
      ...
      foot.innerHTML = `
        ...
        <a href="checkout.html" class="pp-btn pp-btn--primary pp-btn--block" style="justify-content:center;text-decoration:none;">
